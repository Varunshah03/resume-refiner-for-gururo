...
async function extractTextFromDOCX(buffer) {
  try {
    ...
    console.log("Extracted DOCX text (first 500 chars):", result.value.substring(0, 500));
    return result.value;
  } catch (error) {
...
function extractJobTitleFallback(resumeText, fileName = '') {
  const jobTitleKeywords = [
    'software engineer', 'data scientist', 'product manager', 'nurse', 'lawyer',
    'ux designer', 'marketing manager', 'financial analyst', 'teacher', 'digital marketing',
    'product designer'
  ];
  ...
  if (!jobTitle && fileName) {
    ...
    jobTitle = fileNameLower.includes('software') ? 'Software Engineer' :
               ...
               fileNameLower.includes('product designer') ? 'Product Designer' :
               'Professional';
  }
...
async function getCachedAnalysis(resumeText, fileName) {
  const hash = createHash('md5').update(resumeText + fileName).digest('hex');
  try {
    const cached = await fs.readFile(path.join(cacheDir, `${hash}.json`), 'utf-8');
...
async function cacheAnalysis(resumeText, fileName, analysis) {
  const hash = createHash('md5').update(resumeText + fileName).digest('hex');
  try {
    await fs.writeFile(path.join(cacheDir, `${hash}.json`), JSON.stringify(analysis));
...
async function makeRequestWithRetry(fn, retries = 3, delay = 1000) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      dailyRequestCount++;
      console.log(`API request successful. Estimated requests used: ${dailyRequestCount}/${DAILY_QUOTA_LIMIT}`);
      if (dailyRequestCount >= DAILY_QUOTA_LIMIT * 0.8) {
        console.warn(`Quota warning: ${dailyRequestCount}/${DAILY_QUOTA_LIMIT} requests used. Approaching daily limit.`);
      }
      return result;
    } catch (error) {
      if ((error.status === 503 || error.status === 429) && i < retries - 1) {
        const retryDelay = error.errorDetails?.find(detail => detail['@type'] === 'type.googleapis.com/google.rpc.RetryInfo')?.retryDelay;
        const delayMs = retryDelay ? parseFloat(retryDelay) * 1000 : delay * Math.pow(2, i);
        console.log(`Retrying API call (attempt ${i + 2}/${retries}) after ${delayMs}ms due to ${error.status} error`);
...
function generateFallbackAnalysis(jobTitle, resumeText, fileName) {
  ...
  const skillKeywords = [
    'javascript', 'python', 'sql', 'leadership', 'communication', 'problem solving',
    'adaptability', 'data analysis', 'project management', 'clinical skills', 'legal writing',
    'patient care', 'negotiation', 'ux design', 'marketing', 'machine learning', 'digital marketing'
  ];
  const coreSkills = skillKeywords
    .filter(keyword => resumeText.toLowerCase().includes(keyword))
    .slice(0, 6)
    .map(skill => skill.charAt(0).toUpperCase() + skill.slice(1))
    .concat(['Technical Skills', 'Communication', 'Problem Solving'].slice(0, 6 - skillKeywords.length));
    ...
    .map((skill, index) => {
      ...
      if (skillLower.includes('technical')) {
        courseTitle = jobTitle.toLowerCase().includes('software') ? 'Advanced Software Development' :
                      ...
                      jobTitle.toLowerCase().includes('product designer') ? 'Advanced Product Design' :
  ...
  return {
    ...
    learningPaths,
    isFallback: true,
    extractionWarning: resumeText ? undefined : "Failed to extract text from resume, using fallback analysis",
...
async function generateAnalysis(resumeText, fileName) {
  ...
  console.log('Resume Text (first 500 chars):', resumeText?.substring(0, 500) || 'No text extracted');
...
`;
  ...
  try {
    ...
    if (!jsonMatch) {
      ...
      return generateFallbackAnalysis(fallbackTitle, resumeText || '', fileName);
    }
    ...
    analysisData.learningPaths = Array.isArray(analysisData.learningPaths) ? analysisData.learningPaths : [];
    ...
    const fallbackTitle = extractJobTitleFallback(resumeText || '', fileName);
    const fallbackData = generateFallbackAnalysis(fallbackTitle, resumeText || '', fileName);
    if (error.status === 429) {
  ...
  try {
    ...
    if (req.file.mimetype === "application/pdf") {
      try {
        ...
        console.warn("PDF text extraction failed, proceeding with fallback analysis");
        resumeText = '';
        extractionWarning = "Failed to extract text from PDF, using fallback analysis";
      }
    } else if (
      ...
      try {
        ...
        console.warn("DOCX text extraction failed, proceeding with fallback analysis");
        resumeText = '';
        extractionWarning = "Failed to extract text from DOCX, using fallback analysis";
      }
    } else {
    ...
    if (!resumeText || resumeText.trim().length < 10) {
      ...
      resumeText = '';
      extractionWarning = "Resume text too short or empty, using fallback analysis";
    }
